...
class NodeScriptLoader implements IScriptLoader {
	...
	public load(moduleManager: IModuleManager, scriptSrc: string, callback: () => void, errorback: (err: any) => void): void {
		...
		if (/^node\|/.test(scriptSrc)) {
			...
			this._readSourceAndCachedData(normalizedScriptSrc, cachedDataPath, recorder, (err: any, data: string | undefined, cachedData: Buffer | undefined, hashData: Buffer | undefined) => {
				if (err) {
					errorback(err);
					return;
				}
	...
	private _readSourceAndCachedData(sourcePath: string, cachedDataPath: string | undefined, recorder: ILoaderEventRecorder, callback: (err?: any, source?: string, cachedData?: Buffer | undefined, hashData?: Buffer) => any): void {
